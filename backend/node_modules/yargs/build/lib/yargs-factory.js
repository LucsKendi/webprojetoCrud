...
export class YargsInstance {
    ...
    command(cmd, detalhes, builder, handler, middlewares, deprecated) {
        argsert('<string|array|object> [string|boolean] [function|object] [function] [array] [boolean|string]', [cmd, detalhes, builder, handler, middlewares, deprecated], arguments.length);
        __classPrivateFieldGet(this, _YargsInstance_command, "f").addHandler(cmd, detalhes, builder, handler, middlewares, deprecated);
        return this;
    }
    commands(cmd, detalhes, builder, handler, middlewares, deprecated) {
        return this.command(cmd, detalhes, builder, handler, middlewares, deprecated);
    }
    commandDir(dir, opts) {
    ...
    describe(keys, detalhes) {
        argsert('<object|string|array> [string]', [keys, detalhes], arguments.length);
        this[kSetKey](keys, true);
        __classPrivateFieldGet(this, _YargsInstance_usage, "f").describe(keys, detalhes);
        return this;
    }
    detectLocale(detect) {
    ...
    example(cmd, detalhes) {
        argsert('<string|array> [string]', [cmd, detalhes], arguments.length);
        if (Array.isArray(cmd)) {
            cmd.forEach(exampleParams => this.example(...exampleParams));
        }
        else {
            __classPrivateFieldGet(this, _YargsInstance_usage, "f").example(cmd, detalhes);
        }
        return this;
    }
    exit(code, err) {
    ...
    option(key, opt) {
        ...
        if (typeof key === 'object') {
            ...
            const desc = opt.describe || opt.detalhes || opt.desc;
    ...
    usage(msg, detalhes, builder, handler) {
        argsert('<string|null|undefined> [string|boolean] [function|object] [function]', [msg, detalhes, builder, handler], arguments.length);
        if (detalhes !== undefined) {
            assertNotStrictEqual(msg, null, __classPrivateFieldGet(this, _YargsInstance_shim, "f"));
            if ((msg || '').match(/^\$0( |$)/)) {
                return this.command(msg, detalhes, builder, handler);
            }
            else {
                throw new YError('.usage() detalhes must start with $0 if being used as alias for .command()');
            }
        }
        else {
