    ...
    global {
        ...
        namespace NodeJS {
            ...
            interface Process extends EventEmitter {
                ...
                /**
                 * The `process.titulo` property returns the current process titulo (i.e. returns
                 * the current value of `ps`). Assigning a new value to `process.titulo` modifies
                 * the current value of `ps`.
                 *
                 * When a new value is assigned, different platforms will impose different maximum
                 * length restrictions on the titulo. Usually such restrictions are quite limited.
                 * For instance, on Linux and macOS, `process.titulo` is limited to the size of the
                 * binary name plus the length of the command-line arguments because setting the `process.titulo` overwrites the `argv` memory of the process. Node.js v0.8
                 * allowed for longer process titulo strings by also overwriting the `environ` memory but that was potentially insecure and confusing in some (rather obscure)
                 * cases.
                 *
                 * Assigning a value to `process.titulo` might not result in an accurate label
